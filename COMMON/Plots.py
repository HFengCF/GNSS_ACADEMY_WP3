...
def generateScatterPlot(PlotConf):
    ...
    plot = plt.scatter(PlotConf["xData"] , PlotConf["yData"], label = PlotConf["Data_Label"], c = PlotConf["colorData"], marker = ".", s = 2)
    plt.scatter(PlotConf["xData2"] , PlotConf["yData2"], label = PlotConf["Data_Label2"], c = PlotConf["colorData2"], marker = ".", s = 2)
    # plt.colorbar(plot)

    plt.xlabel(PlotConf["xLabel"]) 
    plt.ylabel(PlotConf["yLabel"]) 
    plt.title(PlotConf["Title"]) 

    plt.xlim(left = min(PlotConf["xData"]), right = max(PlotConf["xData"]))
    plt.xticks(PlotConf["xTicks"])
