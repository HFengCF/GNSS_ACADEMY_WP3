...
from InputOutput import LeoPosIdx, SatApoIdx
...
def runCorrectMeas(Year,
    ...
    RcvrPosCopXyz = np.zeros(3)
    RcvrRefPosLlh = np.zeros(3)

    Prev_Sod = 0
    Residual_Dict = {}
    Residual_Dict['SatLabel'] = []
    Residual_Dict['SigmaUere'] = []
    Residual_Dict['E'] = [] # Residual for Galileo
    Residual_Dict['G'] = [] # Residual for GPS
    ...
    # ----------------------------------------------------------------------------------------------------------------------------
        ...
        RcvrPosCopXyzCom = computeLeoComPos(Sod, LeoPosInfo)    # Compute the Center of Masses (CoM)
        ...
        if SatPrepro["Status"] == STATUS_OK:
            ...
            SatCorrInfo["SatClk"] = SatClkBias * Const.SPEED_OF_LIGHT
            ...
            RcvrPosCopXyz = RcvrPosCopXyzCom + RcvrApoXyz
            SatCorrInfo["LeoX"] = RcvrPosCopXyz[0]
            SatCorrInfo["LeoY"] = RcvrPosCopXyz[1]
            SatCorrInfo["LeoZ"] = RcvrPosCopXyz[2]
            ...
            FlightTime = (np.linalg.norm(SatComPos - RcvrPosCopXyz) / Const.SPEED_OF_LIGHT)    # Compute Flight Time
            ...
            Apo = computeSatApo(SatLabel, SatComPos, SunPos, SatApoInfo)   # Compute Antenna Phase Offset in ECEF from ANTEX APOs in satellite-body reference frame
            ...
            SatCorrInfo["SatCodeBia"], SatCorrInfo["SatPhaseBia"] = getSatBias(GammaF1F2, SatLabel, SatBiaInfo)   #Get SAtellite Biases in meters
            ...
            if CorrPrevInfo[SatLabel]["SatComPos_Prev"][0] != 0 and CorrPrevInfo[SatLabel]["SatComPos_Prev"][1] and CorrPrevInfo[SatLabel]["SatComPos_Prev"][2]:
                ...
                # ------------------------------------------------------------------------------
                # It is adding meters to nanoseconds, then dtr need to be divided by SPEED OF LIGHT to transform it into nanoseconds
                SatClkBias += SatCorrInfo["Dtr"] / Const.SPEED_OF_LIGHT                   # Apply Dtr to Clock Bias

                # ------------------------------------------------------------------------------

            # NEED TO CONVERT CLOCK BIAS FROM SECONDS TO METERS
            SatCorrInfo["SatClk"] = (SatClkBias)* Const.SPEED_OF_LIGHT

            if SatCorrInfo["Dtr"] != 0 and SatCorrInfo["Dtr"] != Const.NAN:

                SatCorrInfo["SigmaUere"]  = getUERE(Conf, SatLabel)         # Get Sigma UERE from Conf

                SatCorrInfo["CorrCode"] = SatPrepro["SmoothIF"] + SatCorrInfo["SatClk"] + SatCorrInfo["SatCodeBia"]         # Corrected measurements from previous information
                SatCorrInfo["CorrPhase"] = SatPrepro["IF_P"] + SatCorrInfo["SatClk"] + SatCorrInfo["SatPhaseBia"]       # In the statement is miswritten (IF_L)

                SatCorrInfo["GeomRange"] = computeGeoRange(SatCopPos, RcvrPosCopXyz)             # COmpute Geometrical Range

                SatCorrInfo["CodeResidual"] = SatCorrInfo["CorrCode"] - SatCorrInfo["GeomRange"]                          # Comute the first Residual removing the geometrical range (They include Recevier Clock Estimation)
                SatCorrInfo["PhaseResidual"]  = SatCorrInfo["CorrPhase"] - SatCorrInfo["GeomRange"] 

            CorrPrevInfo[SatLabel]["Sod_Prev"] = Sod
            CorrPrevInfo[SatLabel]["SatComPos_Prev"] = SatComPos
        ...
        or float(SatCorrInfo["GeomRange"]) == 0:
            SatCorrInfo["Flag"] = 0
        ...
        # if Prev_Sod == Sod:
        Residual_Dict["SatLabel"].append(SatLabel)
        Residual_Dict["SigmaUere"].append(SatCorrInfo["SigmaUere"])

        if SatLabel[0] == 'E':
            Residual_Dict["E"].append(float(SatCorrInfo["CodeResidual"]))
        elif SatLabel[0] == 'G':
            Residual_Dict["G"].append(float(SatCorrInfo["CodeResidual"]))


        # if Prev_Sod != Sod:
        #     Prev_Sod = Sod

        #     Residual_Dict["SatLabel"].clear()
        #     Residual_Dict["SigmaUere"].clear()
        #     Residual_Dict["E"].clear()
        #     Residual_Dict["G"].clear()
    # try:
        ...
        # SatCorrInfo["RcvrClk"] = estimateRcvrClk(CorrInfo)
    ...
    CorrInfo = estimateRcvrClk(Residual_Dict, CorrInfo)


    return CorrInfo, RcvrPosCopXyz, RcvrRefPosLlh
